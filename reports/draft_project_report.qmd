...
![VIX price since 1993](images/paste-2.png){fig-align="center" width="500"}
...
![Standardized VIX and S&P500 returns since 1993](images/paste-1.png){fig-align="center" width="500"}
...
For the Reddit sentiment feature, raw r/wallstreetbets comments were scored with roBERTa and then aggregated to a daily table. The resulting sentiment file contains one row per `created_date`, with `final_score` representing the daily sentiment signal and `score` representing the average Reddit comment score for that date. We convert `created_date` to a date field, merge the daily sentiment data with market and GPR observations by date, drop dates where the merged modeling inputs are unavailable, and lag `final_score` by one trading day before using it in the RNN so the model does not use same-day sentiment to predict the same day's VIX close.

![ACF and PACF for VIX (Level) and VIX (Log Transformed)](images/AcfPacf.png){fig-align="center" width="500"}
...
![Keras GRU fold-level walk-forward predictions](images/rnn_walk_forward_folds.png){fig-align="center" width="650"}
...
![Keras GRU actual versus predicted VIX log returns](images/rnn_log_return_predictions.png){fig-align="center" width="650"}
...
![Keras GRU training and validation loss by fold](images/rnn_train_validation_loss.png){fig-align="center" width="650"}
